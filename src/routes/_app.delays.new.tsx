...
export const Route = createFileRoute("/_app/delays/new")({
  component: NewDelay,
});
...
function NewDelay() {
  ...
  return (
    <div>
      <PageHeader title="تسجيل تأخير جديد" />
      <Card><CardContent className="p-6">
        <form onSubmit={onSubmit} className="grid gap-5 md:grid-cols-2">
          ...
          <div className="space-y-2"><Label htmlFor="time">وقت الحضور</Label><Input id="time" type="time" /></div>
          <div className="space-y-2 md:col-span-2"><Label htmlFor="notes">ملاحظات</Label><Textarea id="notes" rows={2} /></div>
          <div className="md:col-span-2 flex justify-end gap-3"><Button type="button" variant="outline" onClick={() => navigate({ to: "/delays" })}>إلغاء</Button><Button type="submit">حفظ</Button></div>
        </form>
      </CardContent></Card>
    </div>
  );
